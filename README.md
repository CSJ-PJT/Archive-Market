...
각 역할은 `capacityPerDay`, `wagePerDay`, `productivityScore`를 가지며, 주문 수가 workforce capacity를 초과하면 backlog로 계산합니다. backlog가 커질수록 cancellationRate, claimRate, delayRisk가 증가하고 AI agent recommendation이 인력 증원, 할인 축소, 고위험 주문 보류, 클레임 처리 우선순위 조정을 제안합니다.
