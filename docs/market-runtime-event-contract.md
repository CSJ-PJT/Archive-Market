...
```text
correlationId = root demand/order entry에서 한 번 생성한 값
causationId = 직접 원인이 된 직전 eventId (root event만 null)
```

- 새 주문은 `market_order.root_correlation_id`에 root correlation을 영구 보관한다.
- `CUSTOMER_DEMAND_CREATED` 또는 `MARKET_ORDER_PLACED` 진입 시점 이후 동일 주문의
  profitability, payment, Nexus, Ledger, Logistics, ArchiveOS review/cost event는 이 값을 재사용한다.
- downstream envelope와 `payload`에는 모두 `eventId`, `correlationId`, `causationId`, `orderId`,
  `entityId`, `simulationRunId`, `workdayId`, `settlementCycleId` 키를 포함한다.
- 같은 outbox idempotency key가 다른 correlation으로 재생성되면 거부한다.
...
```json
{
  ...
  "causationId": "REV-20260710-ORDER-PLACED",
