...
| `GET` | `/api/runtime/status` | Market autonomous runtime loop 상태 |
...
`GET /api/runtime/status`는 ArchiveOS가 Market runtime loop의 생존 상태를 read-only로 확인하기 위한 API입니다.
...
ArchiveOS는 `pipelineStatus=LIVE`와 `lastEventAt` 최신성을 기준으로 Market Live Flow가 정체되지 않았는지 판단할 수 있습니다.
